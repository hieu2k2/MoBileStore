    ...
    const handleList = () => {
        (async () => {
            try {
                ...
                await productApi.getCommentAndRatingByProductId(id).then((item) => {
                    const uniqueCommentsAndRatings = {};
                    let totalRating = 0;
                    let ratingCounts = { oneStar: 0, twoStars: 0, threeStars: 0, fourStars: 0, fiveStars: 0 };
    
                    // Lọc dữ liệu để chỉ lấy một bản ghi duy nhất cho mỗi cặp comment và rating có thông tin giống nhau
                    item.commentAndRatingData.forEach((record) => {
                        const { comment_id, comment, rating, username } = record;
                        if (!uniqueCommentsAndRatings[comment_id]) {
                            uniqueCommentsAndRatings[comment_id] = { comment_id, comment, rating, username };
                            totalRating += rating;
                    ...
                    // Chuyển đổi object thành mảng các bản ghi duy nhất
                    const uniqueRecords = Object.values(uniqueCommentsAndRatings);
                    console.log(reviewsCount)
                    // Tính toán số sao trung bình
                    const avgRating = (totalRating / uniqueRecords.length).toFixed(1);
    
                    // Đặt dữ liệu vào state
                    setProductReview(uniqueRecords);
                                                        ...
                                                        <Row gutter={12}>
                                                            ...
                                                            <Col span={16}>
                                                                ...
                                                                <div className="progress_comment">
                                                                    ...
                                                                    <span className="review-count">{reviewsCount?.oneStar || 0}</span>                                                                </div>
                                                            </Col>
                                                        </Row>
                                                    </div>
                                                </div>
